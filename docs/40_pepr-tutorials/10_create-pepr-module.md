...
1. **Create the module**:
   ...
   Use [`pepr init`](../30_user-guide/10_pepr-cli.md#pepr-init) to generate a new module.
...
1. **Create your custom Pepr Capabilities**
   ...
   Now that you have confirmed Pepr is working, you can now create new [capabilities](../30_user-guide/40_capabilities.md). You'll also want to disable the `HelloPepr` capability in your module (`pepr.ts`) before pushing to production. You can disable by commenting out or deleting the `HelloPepr` variable below:
   ...
   _Note: if you also delete the `capabilities/hello-pepr.ts` file, it will be added again on the next [`pepr update`](../30_user-guide/10_pepr-cli.md#pepr-update) so you have the latest examples usages from the Pepr SDK. Therefore, it is sufficient to remove the entry from your `pepr.ts` module
   config._
...
1. **Build and deploy the Pepr Module**
   ...
   `pepr deploy` - you can use this command to build your module and deploy it into any K8s cluster your current `kubecontext` has access to. This setup is ideal for CI systems during testing, but is not recommended for production use. See [`pepr deploy`](../30_user-guide/10_pepr-cli.md#pepr-deploy) for more info.
