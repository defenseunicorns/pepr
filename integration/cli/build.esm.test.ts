...
describe("build", () => {
  ...
  describe("when building a module with --format esm flag", () => {
    const id = `${FILE.split(".").at(1)}-flag`;
    ...
    describe("for use as an ESM library", () => {
      ...
      it("should not create CJS output files", () => {
        ...
        expect(existsSync(`${testModule}/dist/pepr.js.map`)).toBe(false);
      });
  ...
  describe("when building with auto-detection (type: module)", () => {
    ...
    describe("for use as an ESM library (auto-detected)", () => {
      it("should create .mjs output file", () => {
        expect(existsSync(`${testModule}/dist/pepr.mjs`)).toBe(true);
      });
  ...
  describe("when building with --format cjs flag (explicit CJS)", () => {
    const id = `${FILE.split(".").at(1)}-cjs`;
    const testModule = `${workdir.path()}/${id}`;
    let buildOutput: Result;
    ...
    beforeAll(async () => {
      ...
      const buildArgs = [`--no-embed`, `--format cjs`].join(" ");
      buildOutput = await pepr.cli(testModule, { cmd: `pepr build ${buildArgs}` });
    }, time.toMs("3m"));

    it("should execute 'pepr build --format cjs' successfully", () => {
      expect(buildOutput.exitcode).toBe(0);
      expect(buildOutput.stdout.join("").trim()).toContain("Module built successfully at");
    });

    describe("for use as a CJS library", () => {
      ...
      it("should not create ESM output files", () => {
        expect(existsSync(`${testModule}/dist/pepr.mjs`)).toBe(false);
        expect(existsSync(`${testModule}/dist/pepr.mjs.map`)).toBe(false);
      });
    });
  });

  describe("when CLI format overrides package.json type", () => {
    const id = `${FILE.split(".").at(1)}-override`;
    const testModule = `${workdir.path()}/${id}`;
    let buildOutput: Result;
    ...
    beforeAll(async () => {
      ...
      // Use --format cjs to override the "type": "module" in package.json
      const buildArgs = [`--no-embed`, `--format cjs`].join(" ");
      buildOutput = await pepr.cli(testModule, { cmd: `pepr build ${buildArgs}` });
    }, time.toMs("3m"));
    ...
    it("should create CJS output (CLI flag overrides package.json)", () => {
      expect(existsSync(`${testModule}/dist/pepr.js`)).toBe(true);
      expect(existsSync(`${testModule}/dist/pepr.mjs`)).toBe(false);
    });
  });
});
