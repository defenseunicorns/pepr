...
describe("crd", () => {
  ...
  describe("creates TypeScript types", () => {
    ...
    describe("should produce CRD with correct data", () => {
      ...
      it("should container correct required fields for spec", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.spec.required,
        ).toEqual(["containerPort"]);
      });
      ...
      it("should produce CRD with accurate status conditions lastTransitionTime", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.lastTransitionTime,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.lastTransitionTime.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.lastTransitionTime.format,
        ).toEqual("date-time");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.lastTransitionTime.description,
        ).toContain(
      ...
      it("should produce CRD with accurate status conditions message", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.message,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.message.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.message.description,
        ).toContain(
      ...
      it("should produce CRD with accurate status conditions observedGeneration", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.observedGeneration,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.observedGeneration.type,
        ).toEqual("number");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.observedGeneration.description,
        ).toContain(
      ...
      it("should produce CRD with accurate status conditions reason", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.reason,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.reason.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.reason.description,
        ).toContain(
      ...
      it("should produce CRD with accurate status conditions status", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.status,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.status.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.status.description,
      ...
      it("should produce CRD with accurate status conditions vm", () => {
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.type,
        ).toEqual("object");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.description,
        ).toContain("VM location.");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.name,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.name.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.region,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.region.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.status,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.status.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.message,
        ).toBeDefined();
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.properties!.message.type,
        ).toEqual("string");
        expect(
          crdJSON.spec.versions[0]!.schema!.openAPIV3Schema!.properties!.status.properties!
            .conditions.items!.properties.vm.required,
