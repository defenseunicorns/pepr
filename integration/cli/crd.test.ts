...
describe("crd", () => {
  ...
  describe("creates TypeScript types", () => {
    ...
    describe("npx pepr api generate - generates a CRD from TypeScript types", () => {
      ...
      it("CRD is accurate based on TypeScript types file used in generation", async () => {
        try {
          const crd = await fs.readFile(crdFilePath, "utf8");
          const crdJSON = yaml.load(crd) as CustomResourceDefinition;

          // CRD metadata
          expect(crdJSON.metadata.name).toEqual(`${plural}.${group}.pepr.dev`);

          // CRD Spec - Top Level with names
          ...
          // Status conditinos lastTransitionTime is correct
          ...
          // Status conditions message is correct
          ...
          // Status conditions observedGeneration is correct
          ...
          // Status conditions reason is correct
          ...
          // Status conditions status is correct
          ...
          // status required fields are correct
