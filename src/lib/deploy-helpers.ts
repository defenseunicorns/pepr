...
export async function waitForAllDeploymentReplicas(deployments: KubernetesListObject<kind.Deployment>) {
  ...
  for (const deployment of deployments.items) {
    ...
    while (deployment.status?.readyReplicas !== deployment.spec?.replicas) {
      ...
      Log.info(
        `${deployment.metadata?.namespace}/${deployment.metadata?.name} is not ready - ${readyReplicas} ready replicas out of ${deployment.spec?.replicas} spec replicas`,
      );
      await new Promise(resolve => setTimeout(resolve, 5000));
    }

    Log.info(
      `${deployment.metadata?.namespace}/${deployment.metadata?.name} is ready with ${deployment.status?.readyReplicas} ready replicas out of ${deployment.spec?.replicas} spec replicas`,
    );
    readyCount++;
