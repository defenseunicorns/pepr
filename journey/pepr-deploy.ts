...
export function peprDeploy() {
    ...
    it("should ignore resources not in the capability namespaces during mutation", async () => {
      ...
      expect(
        cm.metadata?.annotations?.[
          `${magicString.split("-").slice(1).join("-")}.pepr.dev/hello-pepr`
        ],
      ).toBeUndefined();
  ...
  async function applyLegacyStoreResource() {
    ...
    execSync(
      `yq eval '.metadata.name = "${magicString}-store"' journey/resources/non-migrated-peprstore.yaml > temp.yaml`,
    );
    execSync(
      `cp journey/resources/non-migrated-peprstore.yaml journey/resources/non-migrated-peprstore.yaml.bak`,
    );
    execSync(`mv temp.yaml journey/resources/non-migrated-peprstore.yaml`);
    // Apply the store
    const appliedStore = spawnSync(
      "kubectl apply -f journey/resources/non-migrated-peprstore.yaml",
      {
        shell: true, // Run command in a shell
        encoding: "utf-8", // Encode result as string
        cwd: resolve(cwd, ".."),
    ...
    execSync(
      `cp journey/resources/non-migrated-peprstore.yaml.bak journey/resources/non-migrated-peprstore.yaml`,
    );
